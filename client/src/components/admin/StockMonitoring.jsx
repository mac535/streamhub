        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-slate-200">
            <thead className="bg-slate-50">
              <tr>
                {['itemName', 'category', 'itemCode', 'availableQty', 'damagedQty', 'usedQty', 'consumedQty', 'district', 'brc', 'status'].map((col) => (
                  <th 
                    key={col}
              ...
              {loading ? (
                ...
                stocks.map((stock) => (
                  <tr key={stock.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500">{stock.itemCode || stock.serialNumber || 'N/A'}</td>
